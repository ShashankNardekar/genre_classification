...
def test_kolmogorov_smirnov(data, ks_alpha):
    ...
    for col in columns:
        ...
        if s1.nunique() < 2 or s2.nunique() < 2:
            raise ValueError(f"Column '{col}' has too few unique values to run KS test: "
                             f"{s1.nunique()} in sample1, {s2.nunique()} in sample2")

        ts, p_value = scipy.stats.ks_2samp(s1, s2)

        assert not np.isnan(p_value), f"KS test failed: p-value is NaN for column '{col}'"
        assert p_value > alpha_prime, (
            f"KS test failed for column '{col}': p_value={p_value:.5f} <= alpha_prime={alpha_prime:.5f}"
        )
